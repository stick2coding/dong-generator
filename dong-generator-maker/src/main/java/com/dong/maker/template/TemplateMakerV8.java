...
import com.dong.maker.template.model.*;
import com.dong.maker.template.utils.TemplateMakerUtils;
 ...
 * V7优化
 * 问题修复
 * 1、同配置多次生成，前一次的文件标记为动态，下一次没变化时会重置为静态
 * 2、前一次生成的.ftl文件，在下一次扫描目录时也会被处理，需要过滤掉
 * 3、json文件中fileInfo中，输入路径应该为.ftl文件路径
 * 4、meta.json的生成在项目内，当使用项目根目录进行扫描时，会被处理，需要调整生成目录，放在根目录同级下
 * 优化
 * 1、参数封装，统一入口
 * 2、使用json来配置参数，方便测试
 */
public class TemplateMakerV8 {

    /**
     * 重载方法，先调这个，然后内部再调用实际方法
    ...
    public static long makeTemplate(TemplateMakerConfig templateMakerConfig){
        ...
        return makeTemplate(id, meta, originProjectPath, templateMakerFileConfig, templateMakerModelConfig, templateMakerOutputConfig);
    }
     ...
     * @return
     */
    public static long makeTemplate(Long id, Meta newMeta, String originProjectPath,
        ...
        //拿到项目的绝对路径，这里已经复制过来，那么直接从temp目录中找到当前项目即可，需要处理进行转义（win）
        // loopFile 中设置层级为1，只查找目录下第一层，然后读取目录，如果找不到就抛出异常
        String sourceRootPath = FileUtil.loopFiles(new File(templatePath), 1, null)
                .stream().filter(File::isDirectory).findFirst().orElseThrow(RuntimeException::new).getAbsolutePath();
        ...
        // 获取模型信息主入口
        List<Meta.ModelConfig.ModelInfo> newModelInfoList = getMetaModelInfoList(templateMakerModelConfig);

        // 配置文件目录，生成位置为根目录同级别
        String metaOutputPath = templatePath + File.separator + "meta.json";
        writeMetaFile(newMeta, metaOutputPath, newFileInfoList, newModelInfoList, sourceRootPath, templateMakerOutputConfig);
    ...
    private static void writeMetaFile(Meta newMeta, String metaOutputPath,
        ...
        if (templateMakerOutputConfig != null){
            // 文件组内外去重
            if (templateMakerOutputConfig.isRemoverGroupFilesFromRoot()){
                List<Meta.FileConfig.FileInfo> fileInfoList = outputMeta.getFileConfig().getFiles();
                outputMeta.getFileConfig().setFiles(TemplateMakerUtils.removeGroupFilesFromRoot(fileInfoList));
            }
        }

        // 输出元信息文件
        FileUtil.writeUtf8String(JSONUtil.toJsonPrettyStr(outputMeta), metaOutputPath);
    }

    /**
     * 获取元信息（meta.json）中全部的模型信息
     * @param templateMakerModelConfig
     * @return
     */
    private static List<Meta.ModelConfig.ModelInfo> getMetaModelInfoList(TemplateMakerModelConfig templateMakerModelConfig) {
    ...
    private static List<Meta.FileConfig.FileInfo> makerFileTemplates(TemplateMakerFileConfig templateMakerFileConfig, TemplateMakerModelConfig templateMakerModelConfig, String sourceRootPath) {
        ...
        //如果文件配置列表为空也直接返回
        List<TemplateMakerFileConfig.FileInfoConfigV2> fileInfoConfigList = templateMakerFileConfig.getFileInfoConfigList();
        if (CollUtil.isEmpty(fileInfoConfigList)){
            return newFileInfoList;
        }

        //更新 首先要先对文件按照过滤规则进行过滤
        for (TemplateMakerFileConfig.FileInfoConfigV2 fileInfoConfig : fileInfoConfigList){
            ...
            // 这里同一目录，可能之前已经制作过，那就有已经存在的.ftl文件，这里需要将这些去除掉，否则就把.ftl文件再次制作为.ftl.ftl文件
            fileList = fileList.stream().filter(file -> !file.getName().endsWith(".ftl")).collect(Collectors.toList());
            //过滤后直接都是文件，直接进行过滤即可
            for (File file : fileList){
                // 可能会出现一次对很多文件进行模板制作，抽取抽象方法来使用
                Meta.FileConfig.FileInfo fileInfo = makeTemplateFile(sourceRootPath, file, templateMakerModelConfig, fileInfoConfig.getCondition());
     ...
     */
    private static Meta.FileConfig.FileInfo makeTemplateFile(String sourceRootPath, File inputFile,
                                                             TemplateMakerModelConfig templateMakerModelConfig,
                                                             String condition){
        // 获取绝对路径 win系统下需要进行转义
        ...
        if(!hasTemplateFile){
            if (contentEquals){
                //没有模板且内容一致，说明是静态文件
                // 此处输入路径指的是原文件（fileInputPath），不是ftl文件（fileOutputPath）
    ...
    /**
     * 问题：测试太麻烦了，每次都要写大量的参数封装代码
     * 解决，将参数封装起来。通过json文件直接生成配置，然后调用方法
     * @param args
     */
...
//        Meta.ModelConfig.ModelInfo modelInfo = new Meta.ModelConfig.ModelInfo();
        ...
        long id = makeTemplate(1861587383960948736L, meta, originProjectPath, templateMakerFileConfig, templateMakerModelConfig, null);
    ...
    private static List<Meta.ModelConfig.ModelInfo> distinctModels(List<Meta.ModelConfig.ModelInfo> modelInfoList) {
        ...
        for (Map.Entry<String, List<Meta.ModelConfig.ModelInfo>> entry : groupKeyModelInfoListMap.entrySet()){
            ...
            //放到带有组信息的新的modelInfo中，从同组中取出一个带有组内信息（groupKey,groupName,condition等）的
...
//        );
    ...
    private static List<Meta.FileConfig.FileInfo> distinctFiles(List<Meta.FileConfig.FileInfo> fileInfoList) {
        ...
        for (Map.Entry<String, List<Meta.FileConfig.FileInfo>> entry : groupkeyFileInfoListMap.entrySet()) {
            ...
            groupKeyFileInfoMergedMap.put(entry.getKey(), newFileInfo);
        }
