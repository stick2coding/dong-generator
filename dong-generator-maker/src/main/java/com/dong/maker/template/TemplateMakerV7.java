...
public class TemplateMakerV7 {
    ...
    public static long makeTemplate(TemplateMakerConfig templateMakerConfig){
        ...
        return makeTemplate(id, meta, originProjectPath, templateMakerFileConfig, templateMakerModelConfig);
    }
...
//        return newFileInfoList;
    ...
    public static long makeTemplate(Long id, Meta newMeta, String originProjectPath, TemplateMakerFileConfig templateMakerFileConfig, TemplateMakerModelConfig templateMakerModelConfig){
        ...
        // 配置文件目录
        // bug 当前生成的met.json文件是在项目根目录下，那么如果后续以根目录为输入路径，去扫描制作模板，就会扫描到met.json，生成meta.json.ftl
        // 修改生成位置为根目录同级别
//        String metaOutputPath = sourceRootPath + File.separator + "meta.json";
     ...
     * 支持一次对多个模型参数进行字符串替换
     *
     * 这里有个问题，就是如果一个文件第一次已经制作了模板，且进行了挖坑处理，文件生成类型为动态
     * 但是第二次的时候发现不需要处理，就会把元信息中的文件生成类型改为静态
     *
     * bug
     * 正确的配置，应该是input是FTL文件，方法中放反了，需要替换下
     * "inputPath": "src/com/dong/acm/MainTemplate.java.ftl",
     * "outputPath": "src/com/dong/acm/MainTemplate.java",
